...
st.title("Propagation Mapping Toolbox (v0.1)")
...
if nii_files and st.button("▶️ RUN PARCELLATION"):
    ...
    with st.spinner("Parcellating images..."):
        ...
        if conn_path.exists():
            ...
            st.session_state.masked_df.columns = [
                clean_name(Path(f).name) for f in nii_files
            ]
...
if st.session_state.masked_df is not None:
    ...
    if st.button("🚀 LAUNCH PROPAGATION & SPIN TEST"):
        ...
        for idx, col in enumerate(df.columns):
            ...
            # observed
            prod_obs = 0.5 * ((act_raw[:, None] * W_full) +
                              (act_raw[:, None] * W_full).T)
...
if st.session_state.get('propagation_results'):
    ...
    for r in st.session_state.propagation_results:
        ...
        residuals = act_vec - masked_vec
        residuals_dict[name] = residuals

        res_summary.append([
            name, r2_f, mae_f, rmse_f,
            mean_delta, p_delta,
            r2_m, mae_m, rmse_m
        ])
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
        ...
        for r in st.session_state.propagation_results:
            ...
            for m_type, m_data in zip(
                ['raw', 'masked', 'zscore'],
                [r['raw_mat'], r['masked_mat'], r['z_mat']]
            ):
                ...
                save_df.to_csv(
                    csv_buffer,
                    sep=sep,
                    index=use_header_index,
                    header=use_header_index
                )
        ...
        zipf.writestr("propagation_maps/summary_metrics.csv",
                      summary_buffer.getvalue())

        # residuals
        resid_buffer = io.StringIO()
        residuals_df.to_csv(resid_buffer, sep=sep)
        zipf.writestr("propagation_maps/residuals.csv",
                      resid_buffer.getvalue())
